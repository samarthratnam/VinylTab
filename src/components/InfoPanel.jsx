...
export default function InfoPanel({
  ...
  const total = track.total > 0 ? track.total : 0;
  const pct = total > 0 ? Math.min(100, Math.max(0, (elapsed / total) * 100)) : 0;
  const seekValue = Math.round((pct / 100) * 1000);

  return (
    <div className="info-panel">
      <section className="player-meta-card">
        ...
        <div className="progress-section" style={{ '--seek-pct': `${pct}%` }}>
          ...
          <div className="times">
            ...
            <span>{track.duration || fmt(total)}</span>
          </div>
        </div>
      </section>
      ...
      <section className="queue-section">
        <div className="queue-head">
          ...
          <div className="queue-count">{tracks.length} tracks</div>
        </div>
        <TrackList tracks={tracks} currentIdx={currentTrackIdx} onSelect={onSelectTrack} className="track-list--main" />
      </section>
    </div>
  );
}
